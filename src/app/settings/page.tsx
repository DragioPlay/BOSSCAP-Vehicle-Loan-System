...
function VehicleFormModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
        <button className="float-right text-xl text-gray-900" onClick={onClose}>
          &times;
        </button>
        <h3 className="text-lg font-bold mb-4 text-gray-900">{isEdit ? "Edit Vehicle" : "Add Vehicle"}</h3>
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-col gap-1">
            <input 
              ...
              className={`border px-2 py-1 rounded text-gray-900 ${errors.model ? 'border-red-500' : ''}`} 
              placeholder="Model*" 
            />
          ...
          <div className="flex flex-col gap-1">
            <input 
              ...
              value={form.trim} 
              onChange={handleChange} 
              className={`border px-2 py-1 rounded text-gray-900 ${errors.trim ? 'border-red-500' : ''}`} 
          ...
          <div className="flex flex-col gap-1">
            <input 
              ...
              value={form.vin} 
              onChange={handleChange} 
              className={`border px-2 py-1 rounded text-gray-900 ${errors.vin ? 'border-red-500' : ''}`} 
              placeholder="VIN*" 
            />
  ...
  return (
    <div className="font-sans flex flex-col items-center justify-start min-h-screen bg-gradient-to-br from-gray-50 to-gray-200">
      <main className="w-full max-w-4xl mt-16 p-8 rounded-3xl shadow-xl bg-white/80 flex flex-col gap-6">
        ...
        <h1 className="text-3xl font-bold text-center text-gray-900">Vehicle Settings</h1>
        ...
        {loading ? (
          <div className="text-gray-900">Loading vehicles...</div>
        ) : (
          <ul className="divide-y divide-gray-200">
            {vehicles.map(v => (
              <li key={v.vehicle_id} className="flex justify-between items-center py-3">
                <div>
                  {v.nickname && <span className="font-semibold italic text-gray-900">{v.nickname} </span>}
                  <span className="ml-2 font-semibold text-gray-900">{v.model}</span>
            ...
            {vehicles.length === 0 && (
              <li className="py-6 text-center text-gray-500">No vehicles found</li>
            )}
          </ul>
        )}
      </main>
